...
func download_file(s string) {
	...
	if JSONerr!=nil{
		r.ToFile(s)
		fmt.Println("Successfully downloaded " + s)
		return
...
func main() {
	if len(os.Args)==1{
		...
		if string(os.Args[1])=="help"{
			show_help("ALL")
		} else if string(os.Args[1])=="push"{
			if len(os.Args)!=3{
				show_help("push")
			} else {
				upload_file(string(os.Args[2]))
				return
			}
		} else if string(os.Args[1])=="pull"{
			if len(os.Args)!=3{
				show_help("pull")
			} else {
				download_file(string(os.Args[2]))
				return
			} 
		}
	}
